...
#TODO: Incorporate this in old file only, makes things unnecessary complicated and multiple copies
...
def incremental_learning():
    ...
    language = {"arabic": {"data": "data", "label": "label", "train_data_path": "../data/arabic/arabic_train.tsv",
                ...
                "english": {"data": "data", "label": "label", "train_data_path": "../data/english/english_train.tsv",
                           "eval_data_path": "../data/english/english_val.tsv", "test_data_path": "../data/english/english_test.tsv",
                              "offensive_label": "OFF"},
    ...
    for i in range(0, 11):
        ...
        if args.no_finetuning:
            ...
            if args.model_type == "shebert":
                tokenizer = BertTokenizer.from_pretrained("./models/slovenian/pre-train/shebert/vocab.txt")
                config = BertConfig.from_pretrained("./models/slovenian/pre-train/shebert//config.json",
                                                    num_labels=len(label_set))
                model = BertForSequenceClassification.from_pretrained("./models/slovenian/pre-train/shebert/pytorch_model.bin",
                                                                      config=config)
            elif args.model_type == "mbert":
                tokenizer = BertTokenizer.from_pretrained('bert-base-multilingual-cased', do_lower_case=True)
                config = BertConfig.from_pretrained("./models/slovenian/pre-train/config.json",
                                                    num_labels=len(label_set))
                model = BertForSequenceClassification.from_pretrained("./models/slovenian/pre-train/pytorch_model.bin",
                                                                      config=config)
            else:
                print("Wrong argument value for model type")
                sys.exit()
